...
import {createActivityChallenge, createActivityCustomWorkout} from "@/services/activity.service";
...
const WorkoutSummaryScreen: React.FC = () => {
    ...
    const {workoutId,totalExercises,totalTime,totalCalories,completedExercises} = useLocalSearchParams();
    ...
    async function hadleCreateActivity(){
        try {
            setLoading(true);
            //@ts-ignore
            await createActivityCustomWorkout(activity,images,workoutId,`${completedExercises}/${totalExercises}`);
            setModalSucces(true)
        }catch (e){
            console.error(e);
            // Optional: Tambahkan error handling atau toast notification di sini
        }finally {
            setLoading(false);
        }
            ...
            <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={styles.statsGrid}>
                    <View style={styles.statCard}>
                        ...
                        <Text style={styles.statNumber}>{`${completedExercises}/${totalExercises}`}</Text>
                ...
                <View>
                    <InputTheme
                        style={{width:"100%"}}
                        placeholder={"masukan nama latihan anda pagi hari ini"}
                        value={activity?.title || ""}
                        onChangeText={(text)=>{setActivity(draft => {draft.title=text})}}
                    />
                    <TextArea
                        placeholder={"Masukan Deskripsi anda"}
                        value={activity?.description || ""}
                        onChangeText={(text)=>{setActivity(draft => {draft.description=text})}}
                    />
                    <ImagePicker
                    ...
                    <Button
                        color={Colors.light.primary}
                        handlePress={hadleCreateActivity}
                        label={"Simpan"}
                        disabled={loading}
                    />
                </View>
